      ...
      {incident && <>
        ...
        <div className="content-grid">
          ...
          <div className="right-column">
            ...
            <section className="card risk-breakdown-card"><CardTitle icon={<FileSearch />} label="SEVERITY EVIDENCE" title="Why this risk score?" /><div className="risk-breakdown"><div className="risk-donut" style={{ "--risk": incident.score }}><div className="risk-donut-center"><strong>{incident.score}</strong><span>/ 100</span></div></div><div className="risk-legend">{incident.factors.length ? incident.factors.map((factor, index) => <div className="risk-legend-row" key={factor.label}><span className={`risk-dot dot-${index % 5}`} /><span>{factor.label}</span><b>+{factor.points}</b></div>) : <p>No critical escalation factors found.</p>}</div></div><p className="risk-explain">Score is calculated from correlated security evidence and escalation factors shown above.</p></section>
