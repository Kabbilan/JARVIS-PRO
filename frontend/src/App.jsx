      ...
      {incident && <>
        ...
        <div className="content-grid">
          ...
          <div className="right-column">
            ...
            <section className="card risk-breakdown-card"><CardTitle icon={<FileSearch />} label="SEVERITY EVIDENCE" title="Risk score composition" />{(() => { const additions = incident.factors.reduce((sum, factor) => sum + factor.points, 0); const base = Math.max(0, incident.score - Math.min(additions, incident.score)); const parts = [{ label: "Base alert severity", points: base }, ...incident.factors.map((factor) => ({ label: factor.label, points: Math.min(factor.points, Math.max(0, incident.score)) }))].filter((part) => part.points > 0); const total = parts.reduce((sum, part) => sum + part.points, 0) || 1; let cursor = 0; const stops = parts.map((part, index) => { const start = cursor; cursor += (part.points / total) * 100; return `var(--risk-c${index % 6}) ${start}% ${cursor}%`; }).join(", "); return <><div className="risk-breakdown"><div className="risk-donut factor-donut" style={{ background: `conic-gradient(${stops})` }}><div className="risk-donut-center"><strong>{incident.score}</strong><span>RISK SCORE</span></div></div><div className="risk-legend">{parts.map((part, index) => <div className="risk-legend-row" key={`${part.label}-${index}`}><span className={`risk-dot factor-dot-${index % 6}`} /><span>{part.label}</span><b>{Math.round((part.points / total) * 100)}% <small>({part.points} pts)</small></b></div>)}</div></div><p className="risk-explain">Each slice shows how much that detected factor contributes to this incident score. Percentages are normalized to the displayed score.</p></>; })()}</section>
